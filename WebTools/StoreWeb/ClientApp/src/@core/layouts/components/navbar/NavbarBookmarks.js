...
const NavbarBookmarks = props => {
  ...
  // ** ComponentDidMount
  useEffect(() => {
    dispatch(getBookmarks({ empID: getUserData().userId }))
  }, [])
  ...
  const handleBookmarkUpdate = ({ id, isBookmarked }) => dispatch(updateBookmarked({ empID: getUserData()?.userId, programID: id, flag: !isBookmarked }))
    ...
    <Fragment>
      <ul className='nav navbar-nav bookmark-icons'>
        ...
        <NavItem className='nav-item d-none d-lg-block'>
          <NavLink className='bookmark-star p-0' onClick={handleBookmarkVisibility}>
            <Icon.Star className='ficon text-warning me-1' size={20} />
          </NavLink>
                    ...
                    >
                      ...
                      >
                        <div className='d-flex justify-content-start align-items-center overflow-hidden'>
                          ...
                          <span className='ps-1 text-truncate'>{item.title}</span>
                        </div>
                      </Link>
                      <Icon.Star
                        size={17.5}
                        ...
                        onClick={() => handleBookmarkUpdate({ id: item.id, isBookmarked: item.isBookmarked })}
                      />
                    </li>
                  )
                }}
