...
const VerticalNavMenuGroup = forwardRef(({
  item,
  groupOpen,
  menuHover,
  activeItem,
  ...
  const toggleOpenGroup = (item, parent) => {
    ...
    if (openGroup.includes(item.id)) {
      ...
      if (!activeGroup.includes(item.id) && currentActiveGroup.includes(item.id)) {
        ...
        if (parent) {
          const indexToKeep = activeGroup.findIndex(item => item === parent.id)
          activeGroup.splice(indexToKeep + 1)
          activeGroup.splice(0, indexToKeep)
        }
  ...
  return (
    ...
    >
      ...
      <ul className='menu-content'>
        ...
        // isOpen={
        //   (groupOpen.length === 0 && groupActive && groupActive.includes(item.id)) ||
        //   (groupOpen && groupOpen.includes(item.id))
        // }
        >
