...
--*********************************************************
-- Name：[POVWeb].[uspTransferOutAddItem]
-- Desc：調出作業-新增明細
-- Author：MoonFeng
-- LastModifyDTM：2022/11/4 15:56
-- Modify History：
-- 1. Initial. MoonFeng@2022/11/4 15:56
-- 
-- SAMPLE：
/*
...
ALTER PROCEDURE [POVWeb].[uspTransferOutAddItem] 
(
	@Editor VARCHAR(16)
...
BEGIN
	...
	--調出作業
	--FactStore = 調入庫點, TranInStore = 中途倉, TranOutStore = 調出庫點
	DECLARE 
	...
	-- 檢查是否有新增的權限
	IF NOT (POVWeb.udfHadAuthority('店櫃調出', @ChangePerson, 'Insert') = 1) BEGIN
	...
	*/
	...
	DECLARE
		...
		, @Mode AS VARCHAR(1) = '1' -- 調出作業，Mode = 1
	...
	BEGIN TRY
		...
		VALUES(
			...
			, @TotalTranNum, @TranPrice, @TranAmount, @TranCost
			, @ChangeDate, @ChangePerson, @SheetFlag, @RemarkNew, @Mode
			--TranOutStore = 調出庫點, TranInStore = 中途倉, FactStore = 調入庫點
			, @NEW_TranOutStore, @HalfWayStore, @NEW_TranInStore
			)
		...
		VALUES(
			@GoodID, @SizeNo, @NEW_TranOutStore
			, @TranNum1, @TranNum2, @TranNum3, @TranNum4, @TranNum5, @TranNum6, @TranNum7, @TranNum8, @TranNum9, @TranNum10
			, @TranNum11, @TranNum12, @TranNum13, @TranNum14, @TranNum15, @TranNum16, @TranNum17
			, @TotalTranNum, @TranAmount
			, @ChangeDate, @ChangePerson, '', 'POVWeb.TransferOut.Add：' + @TransferID
			)
	END TRY
	BEGIN CATCH
		--SELECT 
